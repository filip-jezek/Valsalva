...
pbm_avst = h5read(dl_arst, '/brachial_pressure_mean')/mmHg2SI;
pb_avst = h5read(dl_arst, '/brachial_pressure')/mmHg2SI;
hr_avst = h5read(dl_arst, '/HR')/bpm2SI;
plv_avst = h5read(dl_arst, '/P_LV')/mmHg2SI;
vlv_avst = h5read(dl_arst, '/heartComponent/ventricles/V_LV')/ml2SI;
pow_lv_avst = h5read(dl_arst, '/heartComponent/ventricles/power_LV');
...
plot(vlv(i_int_n), plv(i_int_n), 'b');
plot(vlv_avst(i_int_avst), plv_avst(i_int_avst), 'r');
xlim([50, 180])
ylim([0, 180])
ylabel('mmHg')
xlabel('ml')
...
plot(t_n(i_int_n), pb(i_int_n), 'b');
plot(t_n(i_int_n), pbm(i_int_n), 'b--', 'LineWidth', 0.5);

plot(t_avst(i_int_avst), pb_avst(i_int_avst), 'r');
...
plot(vlv(i_int_n), plv(i_int_n), 'b');
plot(vlv_avre(i_int_avre), plv_avre(i_int_avre), 'r');
xlim([50, 250])
ylim([0, 120])
ylabel('mmHg')
xlabel('ml')
...
plot(t_avre(i_int_avre), pb_avre(i_int_avre), 'r');
...
%%
exportgraphics(gcf,'fig_R_ValvePatho.png','Resolution',300)
