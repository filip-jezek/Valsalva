...
pbp = fill([t;flipud(t)], [pbs;flipud(pbd)], [0, 0.6, 1]);
pbpm = plot(t(i_int), pbm(i_int), 'b', 'LineWidth', 2);
phr = plot(t, hr, 'r');
legend([pbp(1), phr], 'P BA [mmHg]', 'HR [BPM]', 'Location', 'NorthEast');
set(gca,'xtick',[])
xlim([0,120])
ylim([60, 130])
s1.Clipping = 'off';
...
set(gca,'xtick',[])
legend('LV SV', 'RV SV', 'Location', 'NorthEast')
xlim([0,120])
ylabel('ml')
s2.Clipping = 'off';
...
ylabel('mmHg')
xlabel('time')
plot([t(i_hut), t(i_hut)], [65, 0], 'k--')
plot([t(i_hutStop), t(i_hutStop)], [65, 0], 'k--')
ylim([0, 12])
s3.Clipping = 'off';
