...
def getObjectives(vars_set):
    ...
    fun_lib.checkSimulationLength(vars_set['time'][-1],45, penalty=1000)
    # we have 3 intervals
    ...
    #     distance)
    # pwv = 10
    (E_A, A_s) = fun_lib.calculateEA(vars_set['heartComponent.mitralValve.q_in.q'],vars_set['heartComponent.sa_node.cardiac_cycle'], time, interval)
    # vla_peak_frac = q_mv_Ppassive/q_mv_Patrial
    # (atrial_kick, q_mv_saddle) = fun_lib.calculateQdot_mv(vars_set['V_LV'], vars_set['q_mv'], time, intervalQMV)
    # peak pressure drop on aortic valve
    dp_av = numpy.max(vars_set['heartComponent.sa.pressure'][interval]) - numpy.max(vars_set['heartComponent.ventricles.P_LV'][interval])

    # build costs
    ov = [  
        # MRI
            ('BPs', max(vars_set['brachial_pressure'][interval])/mmHg2SI, 120, None, 0.5),
            ('BPd', min(vars_set['brachial_pressure'][interval])/mmHg2SI, 80, None, 0.5),
            ('CO', numpy.mean(vars_set['CO'][interval]) /lpm2SI, 5.76, None, 0.05),
            ...
            ('Ppv', numpy.mean(vars_set['P_pv'][interval])/mmHg2SI, 8, None, 0.5),
            ('ESV_la', numpy.min(vars_set['heartComponent.la.volume'][interval])/ml2SI, 12, None, 5),
            ('EDV_la', numpy.max(vars_set['heartComponent.la.volume'][interval])/ml2SI, 41, None, 5),
            ('dp_av', dp_av/mmHg2SI, None, [-5, 5], 0.1),
            ('E/A', E_A, 1.7, None, 0.5),
            ('E/S', A_s, 2, None, 1.5),
            ('Ts', max(vars_set['heartComponent.aorticValve.Ts'][interval])*1000, 292, None, 30),
            ...
            # 70% exercise
            ('BPs_Ex', max(vars_set['brachial_pressure'][interval_ex])/mmHg2SI, 194.4, None, 1),
            ('BPd_Ex', min(vars_set['brachial_pressure'][interval_ex])/mmHg2SI, 81, None, 2),
            ('Ppas_Ex', numpy.max(vars_set['P_pa'][interval_ex])/mmHg2SI, 34, None, 5),
            ('Ppad_Ex', numpy.min(vars_set['P_pa'][interval_ex])/mmHg2SI, 15, None, 5),
            ...
            ('CO_mEx', numpy.mean(vars_set['CO'][interval_mex]) /lpm2SI, 20.6, None, 0.1),
            ('Ppv_mEx', numpy.mean(vars_set['P_pv'][interval_mex])/mmHg2SI, 24.8, None, 1),
